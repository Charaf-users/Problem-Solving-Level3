...
bool DeleteClientAccount(string AccountNumber, vector<Sclient>&vClient) {
	...
	if (FindDataClientsFromfile(AccountNumber, vClient, client)) {
		...
		cout << "Are you surev you want to delete this Account?(y/n) : ";
		cin >> answer;
		...
		if (answer == 'y' || answer == 'Y') {
			...
			vClient = LoadClientsDataDromFile(clientsFileName);

			cout << "the Account delete succesfully\n" << endl;
			return true;
		}
		...
		cout << "the account number (" << AccountNumber << ")is not found try again :(" << endl;
		return false;
...
int main() {
	...
	DeleteClientAccount(AccountNumber, vClient);
